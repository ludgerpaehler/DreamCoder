...
let supervised_task ?(timeout = 0.001) name ty examples =
  {
    name;
    task_type = ty;
    log_likelihood =
              ...
              try
                match
                  run_for_interval timeout (fun () ->
                      Core.Poly.equal (run_lazy_analyzed_with_arguments p xs) y)
                with
                | Some true -> loop e
                | _ -> false
              with
...
let recent_logo_program :
    (program
    * ((int, Bigarray.int8_unsigned_elt, Bigarray.c_layout) Bigarray.Array1.t * float) option)
    option
    ref =
  ref None
  ...
  | _ ->
      let bx =
        run_for_interval timeout (fun () ->
            ...
            if not (LogoLib.LogoInterpreter.logo_contained_in_canvas l) then None
            else
              ...
              | None ->
                  ...
                  Some bx)
      in
      let bx = match bx with Some (Some bx') -> Some bx' | Some None | None -> None in
      recent_logo_program := Some (program, bx);
      bx

let (_ : unit) =
  register_special_task "LOGO" (fun extras ?(timeout = 0.001) name ty examples ->
      let open Yojson.Basic.Util in
      let cost_matters = try extras |> member "costMatters" |> to_bool with _ -> assert false in

      let by =
        match examples with
        | [ ([ 0 ], y) ] -> Bigarray.(Array1.of_array int8_unsigned c_layout (Array.of_list y))
        | [ ([ 1 ], y) ] -> Bigarray.(Array1.of_array int8_unsigned c_layout (Array.of_list y))
  ...
  register_special_task "differentiable" (fun extras ?(timeout = 0.001) name ty examples ->
      let open Yojson.Basic.Util in
      let maybe_float name default = try extras |> member name |> to_number with _ -> default in
      let maybe_int name default = try extras |> member name |> to_int with _ -> default in
      ...
      let lossThreshold =
        try Some (extras |> member "lossThreshold" |> to_number) with _ -> None
      in
      ...
      let proportional = try extras |> member "proportional" |> to_bool with _ -> false in
      ...
      let examples =
        examples
        |> List.map ~f:(fun (xs, y) ->
               (List.map2_exn argument_types xs ~f:placeholder_data, placeholder_data return_type y))
      in
      ...
      let loss = polymorphic_sse ~clipOutput ~clipLoss return_type in
      {
        ...
        log_likelihood =
          (fun expression ->
            ...
            if List.length parameters > maxParameters || List.length parameters > actualParameters
            then log 0.
            else
              let p = analyze_lazy_evaluation p in
              ...
              | Some l -> (
                  ...
                  let l =
                    if proportional && List.length parameters > 0 then (
                      ...
                      let l = l *& ~$(1. /. n) in
                      let l =
                        restarting_optimize (rprop ~lr ~decay ~grow) ~attempts:restarts ~update:0
                  ...
                  | Some t -> if l < t then 0. -. (d *. parameterPenalty) else log 0.));
      });

  register_special_task "stringConstant"
    (fun
      extras
    ...
    ->
      ...
      let maybe_int name default = try extras |> member name |> to_int with _ -> default in
      let stringConstants =
        extras |> member "stringConstants" |> to_list |> List.map ~f:to_string
...

      {
        ...
        log_likelihood =
          (fun expression ->
            if number_of_string_constants expression > maxParameters then log 0.
            else
              ...
              |> List.map ~f:(fun p ->
                     ...
                     if hit then lc *. Float.of_int (string_constants_length p) else log 0.)
              |> List.fold_right ~init:(log 0.) ~f:Float.max);
      })
...
let keep_best_programs_in_frontier (k : int) (f : frontier) : frontier =
  {
    request = f.request;
    programs =
      List.sort ~compare:(fun (_, a) (_, b) -> Float.compare b a) f.programs |> flip List.take k;
  }
...
let score_programs_for_task (f : frontier) (t : task) : frontier =
  {
    ...
    programs =
      ...
      |> List.filter_map ~f:(fun (program, descriptionLength) ->
             ...
             let open Float in
             if likelihood > -0.1 then Some (program, descriptionLength +. likelihood) else None);
  }
...
let enumerate_for_tasks (g : contextual_grammar) ?(verbose = true) ~maxFreeParameters
  ...
  (hits |> Array.to_list |> List.map ~f:Heap.to_list, !total_number_of_enumerated_programs)
