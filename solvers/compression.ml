open Core
open Physics [@@warning "-33"]
open Pregex [@@warning "-33"]
open Tower [@@warning "-33"]
open Utils
open Type
open Program
open Enumeration
...
let restrict ~topK g frontier =
  let restriction =
    frontier.programs
    |> List.map ~f:(fun (p, ll) -> (ll +. likelihood_under_grammar g frontier.request p, p, ll))
...
let inside_outside ~pseudoCounts g (frontiers : frontier list) =
  ...
  let update g =
    let weighted_summaries =
      summaries
      |> List.map ~f:(fun ss ->
             let log_weights = ss |> List.map ~f:(fun (l, s) -> l +. summary_likelihood g s) in
    ...
    let possible p =
      Hashtbl.fold ~init:0. s.normalizer_frequency ~f:(fun ~key ~data accumulator ->
          if List.mem ~equal:program_equal key p then accumulator +. data else accumulator)
    in
    let actual p = match Hashtbl.find s.use_frequency p with None -> 0. | Some f -> f in
  ...
  ( g,
    summaries
    |> List.map ~f:(fun ss ->
           ss |> List.map ~f:(fun (l, s) -> l +. summary_likelihood g s) |> lse_list)
    |> fold1 ( +. ) )
...
let grammar_induction_score ~aic ~structurePenalty ~pseudoCounts frontiers g =
  ...
  let production_size = function
    ...
    | Invented (_, e) -> (
        ...
        match e with
        | Apply (Apply (Primitive (_, "fix1", _), i), b) ->
            assert (is_index i);
            program_size b
        | _ -> program_size e)
...
let normalize_invention i =
  ...
  let mapping =
    free_variables i |> List.dedup_and_sort ~compare:( - ) |> List.mapi ~f:(fun i v -> (v, i))
  in
...
let rewrite_with_invention i =
  ...
  let rec visit e =
    if program_equal e i then applied_invention
    else
      match e with
  ...
  fun request e ->
    ...
    with UnificationFailure ->
      if !verbose_compression then (
        ...
        Printf.eprintf
          "Going to proceed as if the rewrite had failed - but look into this because it could be \
           a bug.\n";
        flush_everything ());
...
let compression_worker connection ~inline ~arity ~bs ~topK g frontiers =
  ...
  if !verbose_compression then
    Printf.eprintf
      "(worker) %d distinct version spaces enumerated; %d accessible vs size; vs log sizes: %s\n"
      v.i2s.ra_occupancy
      (frontier_indices |> List.concat |> reachable_versions v |> List.length)
      (frontier_indices |> List.concat
      |> List.map ~f:(Float.to_string % log_version_size v)
      |> join ~separator:"; ");

  let v, frontier_indices =
    garbage_collect_versions ~verbose:!verbose_compression v frontier_indices
  in
  Gc.compact ();
  ...
  let candidate_scores : float list =
    time_it ~verbose:!verbose_compression "(worker) beamed version spaces" (fun () ->
        beam_costs' ~ct:cost_table ~bs candidates frontier_indices)
  in

  send candidate_scores;
  (* I hope that this leads to garbage collection *)
  let[@warning "-26"] candidate_scores = () and[@warning "-26"] cost_table = () in
  Gc.compact ();
          ...
          List.map !frontiers ~f:(fun frontier ->
              let programs' =
                List.map frontier.programs ~f:(fun (originalProgram, ll) ->
                    ...
                    (program', ll))
              in
              { request = frontier.request; programs = programs' })
        in
        new_frontiers)
  in
...
let compression_step_master ~inline ~nc ~structurePenalty ~aic ~pseudoCounts ?(arity = 3) ~bs ~topI
    ~topK g frontiers =
  let sockets = ref [] in
  ...
  if !verbose_compression then
    ignore (Unix.system "ps aux|grep compression 1>&2" : Core.Unix.Exit_or_signal.t);
  ...
  let candidates : program list =
    ...
    |> List.map ~f:(fold1 ( +. ))
    |> List.zip_exn candidates
    |> List.sort ~compare:(fun (_, s1) (_, s2) -> Float.compare s1 s2)
    |> List.map ~f:fst
  in
  ...
  | _ ->
      ...
      let _, initial_score =
        grammar_induction_score ~aic ~structurePenalty ~pseudoCounts
          (frontiers |> List.map ~f:(restrict ~topK g))
          g
      in
      Printf.eprintf "Initial score: %f\n" initial_score;

      let (g', best_score), best_candidate =
        time_it "Scored candidates" (fun () ->
            List.map2_exn candidates new_frontiers ~f:(fun candidate frontiers ->
                (score frontiers candidate, candidate))
      ...
      if Float.( < ) best_score initial_score then (
        ...
        Printf.eprintf "Improved score to %f (dScore=%f) w/ new primitive\n\t%s : %s\n" best_score
          (best_score -. initial_score) (string_of_program new_primitive)
          (closed_inference new_primitive |> canonical_type |> string_of_type);
        flush_everything ();
        (* Rewrite the entire frontiers *)
        let frontiers'' : frontier list =
          time_it "rewrote all of the frontiers" (fun () ->
...
let compression_step ~inline ~structurePenalty ~aic ~pseudoCounts ?(arity = 3) ~bs ~topI ~topK g
    frontiers =
  ...
  let frontier_indices : int list list =
    time_it "calculated version spaces" (fun () ->
        !frontiers
        |> List.map ~f:(fun f ->
               f.programs
               |> List.map ~f:(fun (p, _) ->
  ...
  let candidates =
    ...
    |> List.filter ~f:(fun candidate ->
           ...
           try
             ignore (normalize_invention candidate : program);
...
let export_compression_checkpoint ~nc ~structurePenalty ~aic ~topK ~pseudoCounts ?(arity = 3) ~bs
    ~topI g frontiers =
  let timestamp = Time.now () |> Time.to_filename_string ~zone:Time.Zone.utc in
...
let compression_loop ?(nc = 1) ~structurePenalty ~inline ~aic ~topK ~pseudoCounts ?(arity = 3) ~bs
    ~topI ~iterations g frontiers =
  let find_new_primitive old_grammar new_grammar =
    new_grammar |> grammar_primitives
    |> List.filter ~f:(fun p ->
  ...
  let illustrate_new_primitive new_grammar primitive frontiers =
    let illustrations =
      frontiers
      |> List.filter_map ~f:(fun frontier ->
             let best_program =
               (restrict ~topK:1 new_grammar frontier).programs |> List.hd_exn |> fst
             in
    ...
    illustrations
    |> List.iter ~f:(fun program -> Printf.eprintf "  %s\n" (string_of_program program))
  in
    ...
    if iterations < 1 then (
      ...
      match
        time_it "Completed one step of memory consolidation" (fun () ->
            step ~inline ~structurePenalty ~topK ~aic ~pseudoCounts ~arity ~bs ~topI g frontiers)
      with
      | None -> (g, frontiers)
      | Some (g', frontiers') ->
          illustrate_new_primitive g' (find_new_primitive g g') frontiers';
          if !verbose_compression && iterations > 1 then
            export_compression_checkpoint ~nc ~structurePenalty ~aic ~topK ~pseudoCounts ~arity ~bs
              ~topI g' frontiers';
          flush_everything ();
...
let run_compression () =
  ...
  let j =
    ...
    else Yojson.Basic.from_channel Stdlib.stdin
  in
  ...
  (verbose_compression := try j |> member "verbose" |> to_bool with _ -> false);

  (factored_substitution := try j |> member "factored_apply" |> to_bool with _ -> false);
  if !factored_substitution then
    Printf.eprintf "Using experimental new factored representation of application version space.\n";

  (collect_data := try j |> member "collect_data" |> to_bool with _ -> false);
  if !collect_data then verbose_compression := true;

  let inline = try j |> member "inline" |> to_bool with _ -> true in

  let nc = try j |> member "CPUs" |> to_int with _ -> 1 in

  let iterations = try j |> member "iterations" |> to_int with _ -> 1000 in
  ...
  let g, frontiers =
    if Float.( > ) aic 500. then (
      Printf.eprintf
        "AIC is very large (over 500), assuming you don't actually want to do DSL learning!";
      (g, frontiers))
    else
      compression_loop ~inline ~iterations ~nc ~topK ~aic ~structurePenalty ~pseudoCounts ~arity
        ~topI ~bs g frontiers
  in
